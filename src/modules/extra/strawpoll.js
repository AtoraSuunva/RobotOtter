...
module.exports.events.message = async (bot, message) => {
  ...
  if (!res.ok) {
    ...
    return message.channel.send(
      'Strawpoll returned an error while trying to fetch that',
    )
  }
...
function sortVotes(data) {
  ...
  for (let thing of results) {
    if (sortedByVotes[thing.votes] === undefined)
      sortedByVotes[thing.votes] = []
  ...
  for (let foo of Object.keys(sortedByVotes).reverse()) {
    list += `[${position}] ${sortedByVotes[foo][0]} # ${foo} vote${
      foo == 1 ? '' : 's'
    }\n`

    for (let j = 1; j < sortedByVotes[foo].length; j++) {
      list += `  L ${sortedByVotes[foo][j]} # ${foo} vote${
        foo == 1 ? '' : 's'
      }\n`
    }
  ...
  return {
    embed: {
      author: { name: data.title, url: `http:\/\/www.strawpoll.me/${data.id}` },
      description: list,
      footer: { text: `${totalVotes} vote${totalVotes == 1 ? '' : 's'}` },
    },
  }
}
