module.exports.config = {
  ...
  expandedHelp:
    'conv <num><unit> [to] <unit> #Convert from one unit to another\nconv <num><unit> #Convert from one unit to best\nconv <unit> #What you can convert to',
  usage: [
    'Convert F to C',
    'convert 4f to c',
    'Convert, but less verbose',
    'conv 4f c',
    'Alternatively',
    'conv 3 ml oz',
    'Check valid units',
	...
	*/
    ...
    try {
      if (Number.isNaN(val) && ini !== undefined) {
        ...
        msg =
          '└> `' +
          convert(val).from(ini).to(end) +
          (['f', 'c'].includes(end.toLowerCase()) ? '°' : '') +
          end +
          '`'
      }
    } catch (e) {
      ...
      return message.channel.send(
        `Something went wrong while converting, check your units?:\n ${e}`,
      )
    }
