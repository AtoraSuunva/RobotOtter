...
module.exports.events.message = (bot, message) => {
  ...
  if (coins < 0)
    return message.channel.send(
      prefix + `If I had negative coins I'd be in debt.`,
    )
  if (coins === 0)
    return message.channel.send(
      prefix + `Here's the result! (nothing what did you expect)`,
    )
  if (coins > maxCoins)
    return message.channel.send(
      prefix + `I've only got ${maxCoins} coins. I'm on a tight budget.`,
    )

  const finalFlips = flipCoins(coins)
  const occurences = JSON.stringify(countOccurences(finalFlips))
    .replace(/"/g, '')
    .replace(/([:,])/g, '$1 ')
  const msg = finalFlips.join(', ') + `\nHere's the count: ${occurences}`

  if (msg.length < 2000) message.channel.send(prefix + '\n' + msg)
  else
    message.channel.send(
      prefix + 'The result was too long for me to send. Try something smaller.',
    )
}
...
function countOccurences(arr) {
  const o = {}
  for (let i of arr) o[i] = o[i] ? o[i] + 1 : 1
  return o
}
